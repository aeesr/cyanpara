...
client.on("ready", async () => {
    ...
    setInterval(function() {
        client.user.setPresence({ game: { name: config.prefix + "Ayuda | " + client.guilds.size + " Servidor! y " + client.users.size + " Usuarios!", url: "https://www.twitch.tv/", type: 1 } });
    //Update every 05 seconds
    }, 60 * 1000);

    {client.on("message", (message) =>{
        if (message.content.startsWith("Te amo")) {
            message.channel.send("Y-yo tambien");
    ...
    client.setInterval(() =>{
        ...
        //intente cambiar el color del rol para cada servidor
        for(let i in client.colors) {
            let guildId = client.colors[i].guild;
            let guild = client.guilds.get(guildId);


            //si el servidor se elimina o se bota el bot, elimínelo de la configuración
            if(guild === null) {
...
client.on("message", async message =>{
    ...
    if(command === "rainbow") {
        ...
        msg.success(message, "Multicolor aplicado en **`" + rainbowRole + "`**")
